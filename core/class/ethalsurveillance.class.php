...
class ethalsurveillance extends eqLogic {
  ...
  private function EthcreateCmd() {
        ...
        */
        ...
        /* commande RAZ compteur demarrage */
    ...
    public function ethCumulData($_startDate = null, $_endDate = null) {
      $cmdHistoryFctTotal = $this->getCmd(null, 'tempsfcttotal');
      $cmdHistoryCpt = $this->getCmd(null, 'count');

      if (!is_object($cmdHistoryFctTotal) or !is_object($cmdHistoryCpt)) {
        return array();
      }
      ...
      while (strtotime($date) <= strtotime($_endDate)) {

        $startDate = date("Y-m-d 00:00:00", strtotime($date));        
        $endDate = date("Y-m-d 23:59:59", strtotime($date));

        $dayHistoryFctTotal = $cmdHistoryFctTotal->getStatistique($startDate,$endDate);            
        $cumulDayFctTotal = $dayHistoryFctTotal['max']- $dayHistoryFctTotal['min'];

        $dayHistoryCpt = $cmdHistoryCpt->getStatistique($startDate,$endDate);            
        $cumulDayCpt = $dayHistoryCpt['max'] - $dayHistoryCpt['min'];

        $dataFctTotal[$date] = array(strtotime($startDate. ' UTC') * 1000, 0);
        $dataCpt[$date] = array(strtotime($startDate. ' UTC') * 1000, 0);
        
        $dataFctTotal[$date][1] = $cumulDayFctTotal/3600;
        $dataCpt[$date][1] = $cumulDayCpt;
                
        $date = date ("Y-m-d H:i:s", strtotime("+1 day", strtotime($date)));
        
        //echo $dayHistoryCpt['max'];


        //log::add('ethalsurveillance', 'debug', 'Max History' . $dayHistoryCpt['max'] .'Min History'. $dayHistoryCpt['min'].'cumul'.$cumulDayCpt );

      }
      
      $tmp1 = array_values($dataFctTotal);
      $tmp2 = array_values($dataCpt);


      
      $data = array('tpsFctName' => $cmdHistoryFctTotal->getName(),'tpsFct' => $tmp1,'cptName' => $cmdHistoryCpt->getName(),'cpt' => $tmp2);
      //echo $data;
      return $data;
    }
